...
base_data <- base_data %>%
  ...
  subset(TAXON %in% c("CLAM", "MAYFLY", "ZOOP")) %>%
  summarise(mean_d13C = mean(d13C), mean_d15N = mean(d15N), sd_d13C = sd(d13C), sd_d15N = sd(d15N))

#make dataset w columns for d13C, d15N by baseline (eph) and (zoopclam)
library(reshape2)
...
###Tadenac does not have zoop baseline, so combine Zoop and Clam column for it
base_data$d13C_zoopclam <- base_data$d13C_zoop
base_data$d13C_zoopclam[base_data$Water == "Tadenac Lk."] <- -27.796
base_data$d15N_zoopclam <- base_data$d15N_zoop
base_data$d15N_zoopclam[base_data$Water == "Tadenac Lk."] <- 3.595
