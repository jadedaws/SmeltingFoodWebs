...
SMB_PL_length<-ggplot(x, aes(x=Water2, y=gemmean))+
  ...
  geom_point() +
  ylim(-0.1,1.2) +
  ylab("SMB adjp_lit~Water +/- 95% CI") + 
  #geom_text(aes(label=.group), vjust = -3.0) +
  theme_bw()
...
yp_pl_em
#letters
#mod_means <- cld(yp_pl_em, alpha = 0.05, Letters = letters)
...
#x1<-merge(x1,mod_means)
#x1$.group<-gsub("[[:space:]]", "", x1$.group)
#plot
YP_PL_length<-ggplot(x1, aes(x=Water2, y=gemmean))+
  geom_errorbar(aes(ymin=glower.CL, ymax=gupper.CL), width=0.2)+
  geom_point()+
  ylim(-0.1,1.2) +
  ylab("YP adjp_lit~Water2+length +/- 95% CI") +  
...
smb_tp
#letters
mod_means <- cld(smb_tp, alpha = 0.05, Letters = letters)
x2<-as.data.frame(summary(smb_tp)$emmeans)
x2<-merge(x2,mod_means)
x2$.group<-gsub("[[:space:]]", "", x2$.group)
#plot - need to force Y scale the same
SMB_TP_size<-ggplot(x2, aes(x=Water2, y=emmean))+
  geom_errorbar(aes(ymin=lower.CL, ymax=upper.CL), width=0.2)+
  geom_point()+
  ylim(2.9,4.25) +
  ylab("SMB TP_Water2+length +/- 95% CI") + 
  geom_text(aes(label=.group), vjust = -2.5) +
  theme_bw()
...
yp_tp
#letters
mod_means <- cld(yp_tp, alpha = 0.05, Letters = letters)
x3<-as.data.frame(summary(yp_tp)$emmeans)
x3<-merge(x3,mod_means)
x3$.group<-gsub("[[:space:]]", "", x3$.group)
#plot - need to force Y scale the same
YP_TP_size<-ggplot(x3, aes(x=Water2, y=emmean))+
  geom_errorbar(aes(ymin=lower.CL, ymax=upper.CL), width=0.2)+
  geom_point()+
  ylim(2.9,4.25) +
  ylab("YP TP~Water2+length +/- 95% CI") +
  geom_text(aes(label=.group), vjust = -4.25) +
  theme_bw()

##4 panel plot
plot_grid(SMB_PL_length, YP_PL_length, SMB_TP_size, YP_TP_size, labels = "AUTO")
