...
#write.csv(data, "ellipse-data_Mar04.csv")

#######
# SMB #
#######
#ANOVA w/ only sig. variables
smb_pl_mod<- lm(plit_bound~Water, data=data, subset=(TAXON=="SMB"))
anova(smb_pl_mod)
#contrasts
...
yp_pl_mod<- lm(plit_bound~Water+length, data=data, subset=(TAXON=="YPERCH"))
anova(yp_pl_mod)
...
sea<-data %>%
  subset(select = c("Water","Water2", "TAXON", "p_lit", "adjp_lit","plit_bound", "tp_2")) %>%
...
pl_ref$letter <- c("A", "A", "C", "BC", "AC", "A", "A", "A", "A", "A")
