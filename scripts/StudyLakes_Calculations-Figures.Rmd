...
#plot
ggplot(cond_dat, aes(x = TAXON, y = mean.CN, colour = TAXON, group = TAXON)) +
  geom_point() +
...
yp_cn_aov<-aov(C.N ~ Lake, data = CN_YP)
summary(yp_cn_aov)
smb_cn_aov<-aov(C.N ~Lake, data = CN_SMB)
summary(smb_cn_aov)
TukeyHSD(smb_cn_aov, which = "Lake")
...
#plot
ggplot(FIdat, aes(x = TAXON, y = mean.FI, colour = TAXON, group = TAXON)) +
  geom_point() +
...
yp_fi_aov<-aov(FI ~ Lake, data = FI_YP)
summary(yp_fi_aov)
TukeyHSD(yp_fi_aov, which = "Lake")

smb_fi_aov<-aov(FI ~ Lake, data = FI_SMB)
summary(smb_fi_aov)
TukeyHSD(smb_fi_aov, which = "Lake")
```
