...
SMB_PL<-ggplot(smb_adjPL, aes(x=Water, y=adjprop_litt))+
  geom_point()+
  ylab("SMB Prop Litt +/- sd") +  
  theme_bw()
...
yp_pl_aov<-lm(adjprop_litt ~ Water*FLEN, data = yp_adjPL)
anova(yp_pl_aov)
...
x<-as.data.frame(summary(YP_PLTLEN)$emmeans)

#plot for PL adj
YP_PL<-ggplot(x, aes(x=Water, y=emmean))+
  geom_errorbar(aes(ymin=lower.CL, ymax=upper.CL), width=0.2)+
  geom_point()+
  ylab("YP Prop Litt  +/- sd") +  
  theme_bw()
...
x1<-as.data.frame(summary(smb_tp)$emmeans)

#plot for SMB TP with body size correction - need to force Y scale the same
SMB_TP_size<-ggplot(x1, aes(x=Water, y=emmean))+
  geom_errorbar(aes(ymin=lower.CL, ymax=upper.CL), width=0.2)+
  geom_point()+
  ylab("SMB TP +/- sd (size corr)") +  
  theme_bw()
...
x2<-as.data.frame(summary(yp_tp)$emmeans)

#plot for YP TP with body size correction - need to force Y scale the same
YP_TP_size<-ggplot(x2, aes(x=Water, y=emmean))+
  geom_errorbar(aes(ymin=lower.CL, ymax=upper.CL), width=0.2)+
  geom_point()+
  ylab("YP TP +/- sd (size corr)") +  
  theme_bw()
