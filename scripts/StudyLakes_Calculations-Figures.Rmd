...
adjpl <- 
  ggplot(subset(data, TAXON %in% c("SMB", "YPERCH")), aes(x = TAXON, y = mean.adjpl, colour = TAXON, group = TAXON)) +
  geom_point(show.legend = FALSE) +
...
##SMB
smb_tp_aov<- lm(tp_2~Water*TLEN, data=data, subset=(TAXON=="SMB"))
anova(smb_tp_aov)
...
#YP
yp_tp_aov<- lm(tp_2~Water*TLEN, data=data, subset=(TAXON=="YPERCH"))
anova(yp_tp_aov)
