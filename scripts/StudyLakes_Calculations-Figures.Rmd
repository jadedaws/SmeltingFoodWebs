...
sea<-data %>%
  subset(select = c("Water2", "TAXON", "p_lit", "adjp_lit", "tp_2")) %>%
...
ggplot(sea, aes(x = p_lit, y = tp_2, colour = TAXON, group = interaction(Water2, TAXON))) +
  geom_point() +
  stat_ellipse(type = "norm", level = 0.4, lwd = 1.5) +
  theme_classic(base_size = 14) +
  labs(title = "Raw PL 40% ellipse") +
  scale_colour_manual(name = "Species", labels = c("Smallmouth bass", "Yellow perch"), values = c("#D8B365", "#5AB4AC"))+
  facet_wrap(~Water2)
...
#write.csv(x = sea, file = "newsiberdat.csv")
```
