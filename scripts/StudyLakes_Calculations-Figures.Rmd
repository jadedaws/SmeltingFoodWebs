...
##it WORKS now tidy
test<-left_join(data,LEN,keep=F)
test<- subset(test, select=-c(x_FLEN, x_TLEN))
test$length1 <- (test$FLEN+test$x_diff)
test$length <- coalesce(test$TLEN,test$length1)
test$length <- coalesce(test$length,test$FLEN)
...
#rm(smb_pl_em, yp_pl_em, smb_tp, yp_tp, mod_means)
```
