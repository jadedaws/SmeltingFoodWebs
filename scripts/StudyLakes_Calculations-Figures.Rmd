...
data$p_litnorm <- NA
data$p_litnorm = data$p_lit
data$p_litnorm[data$p_lit>1.5] <- NA
data$p_litnorm[data$p_lit<(-0.5)] <- NA

pn<-data %>%
  group_by(Water) %>%
  filter(!is.na(p_litnorm)) %>%
  mutate(pl_n = ((p_litnorm - min(p_litnorm))/(max(p_litnorm) - min(p_litnorm)))
         ) %>%
...
data_sum <- data %>% group_by(Water, TAXON) %>%
  summarise(mean.pl = mean(p_litnorm), sd.pl = sd(p_litnorm), mean.adjpl = mean(adjp_lit), sd.adjpl = sd(adjp_lit))
#add to df
...
rm(data_sum, pl_in, pl_over, pl_under, num_in, num_over, num_under, num_out)
```
...
#write.csv(data, "JD-data_Feb16.csv")

#######
# SMB #
#######
#ANOVA w/ only sig. variables
smb_pl_mod<- lm(p_litnorm~Water, data=data, subset=(TAXON=="SMB"))
anova(smb_pl_mod)
#contrasts
...
yp_pl_mod<- lm(p_litnorm~Water+length, data=data, subset=(TAXON=="YPERCH"))
anova(yp_pl_mod)
...
sea<-data %>%
  subset(select = c("Water","Water2", "TAXON", "p_lit", "adjp_lit","p_litnorm", "tp_2")) %>%
  filter(TAXON %in% c("SMB", "YPERCH"))
sea$Water2[sea$Water2 == "Baby Lk."]<-"Barren Lake"
sea$Water2[sea$Water2 == "Daisy Lk."]<-"Semi-barren Lake"
sea$Water2[sea$Water2 == "Nelson Lk."]<-"Forested Lake"
sea$Lake <- NA
sea$Lake = sea$Water
sea$Lake[sea$Lake == "Baby Lk."]<-"Barren Lake"
sea$Lake[sea$Lake == "Daisy Lk."]<-"Semi-barren Lake"
sea$Lake[sea$Lake == "Nelson Lk."]<-"Forested Lake"
sea$Lake[sea$Lake == "Mendelssohn"]<-"Reference 1"
sea$Lake[sea$Lake == "Turtleshell"]<-"Reference 2"
sea$Lake[sea$Lake == "Manitou"]<-"Reference 3"
sea$Lake[sea$Lake == "Wawiashkashi"]<-"Reference 4"
sea$Lake[sea$Lake == "Stormy"]<-"Reference 5"

#hist(sea$p_lit)

#40% ellipse with PL (-0.5-1.5, lipid corr)
ggplot(sea, aes(x = p_litnorm, y = tp_2, colour = TAXON, group = interaction(Lake, TAXON))) +
  ...
  zz<- stat_ellipse(data = sea, aes(x = p_litnorm, y = tp_2, colour = TAXON, group = interaction(Water, TAXON), type = "norm", level = 0.4, lwd = 1.5)) 
str(zz)
zz$compute_geom_1

ggplot(sea, aes(x = adjp_lit, y = tp_2, colour = TAXON, group = interaction(Water2, TAXON))) +
  geom_point() +
  stat_ellipse(type = "norm", level = 0.4, lwd = 1.5) +
  theme_classic(base_size = 14) +
...
#write.csv(x = sea, file = "test1.csv")
```

```{r plotting reference lakes individually - Supplemental}
#pl
