...
PL<-ggplot(PL_gm, aes(x=Water2, y=gemmean, colour=TAXON))+
  geom_errorbar(aes(ymin=glower.CL, ymax=gupper.CL), width=0.2, position = position_dodge(0.4),show.legend=F)+
  geom_point(position = position_dodge(0.4))+
  ylim(0,1.1) +
  ylab("Proportion Littoral") + 
  scale_colour_manual(labels = c("Smallmouth bass", "Yellow perch"), values = c("#D55E00", "#E69F00"), guide="none") +
  theme_bw()+
...
TP<- ggplot(TP_gm, aes(x=Water2, y=gemmean, colour=TAXON))+
  ...
  scale_colour_manual(name = "Species", labels = c("Smallmouth bass", "Yellow perch"), values = c("#D55E00", "#E69F00")) +
  theme_bw()+
  theme(axis.title.x = element_blank()) +
  scale_x_discrete(labels = lakelab)
...
rm(smb_pl_mod, yp_pl_mod, smb_tp_mod, yp_tp_mod, smb_pl_em, yp_pl_em, smb_tp, yp_tp)
```
...
PL_gm1 <- PL_gm %>%
  rename("Water" = "Water2",
    "PL" = "gemmean",
         "PL.lwr.CL" = "glower.CL",
         "PL.upr.CL" = "gupper.CL")
sea <- full_join(TP_gm1,PL_gm1)
sea$Water[sea$Water == "Baby Lk."] <- "Barren Lake"
sea$Water[sea$Water == "Daisy Lk."] <- "Semi-barren Lake"
sea$Water[sea$Water == "Nelson Lk."] <- "Forested Lake"
sea$Water[sea$Water == "Reference"] <- "Reference Lakes"

CJFAS_sea<-ggplot(sea, aes(x = PL, y = TP, colour = TAXON, group = interaction(Water, TAXON))) +
  geom_point() +
