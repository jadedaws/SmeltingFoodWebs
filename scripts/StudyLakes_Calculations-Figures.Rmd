...
#w reference CI
PL<- PL1+
  geom_rect(data = refPL_gm, aes(ymin = glower.CL, ymax = gupper.CL, fill=TAXON), xmin = -Inf, xmax = Inf, alpha = 0.25) +
  geom_hline(data = refPL_gm, aes(yintercept = gemmean,linetype = "Reference Lakes mean", colour=TAXON), lwd=1.1)+
     theme(axis.title.x = element_blank()) +
  xlim('Baby Lk.', 'Daisy Lk.', 'Nelson Lk.')
...
#w reference CI
TP<- TP1+
  geom_hline(data = refTP_gm, aes(yintercept = gemmean,linetype = "Reference Lakes mean", colour=TAXON), lwd = 1.1) +
  geom_rect(data = refTP_gm, aes(ymin = glower.CL, ymax = gupper.CL), xmin = -Inf, xmax = Inf, alpha = 0.25) +
  scale_fill_manual(labels = c("Smallmouth Bass", "Yellow perch"), values = c("black", "grey"))+
   theme(axis.title.x = element_blank()) +
  scale_x_discrete(labels = lakelab, limits = c('Baby Lk.','Daisy Lk.','Nelson Lk.'))

##4 panel plot
plot_grid(PL, TP, labels = "AUTO")

rm(smb_pl_mod, yp_pl_mod, smb_tp_mod, yp_tp_mod, smb_pl_em, yp_pl_em, smb_tp, yp_tp, mod_means, x, x1, x2, x3)
```
