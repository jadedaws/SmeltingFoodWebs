...
#Water<-c("Baby Lk.", "Daisy Lk.", "Nelson Lk.", "Manitou", "Mendelssohn", "Stormy", "Turtleshell", "Wawiashkashi")
#size<-c(11.9, 36.1, 308.8, 322.1, 460.5, 333.4, 161.1, 414.4)
#ls <- data.frame(Water,size)
...
PL_gm$colour<-"#999999"
PL_gm$colour[PL_gm$TAXON=="YPERCH"] <- "#E69F00"
refPL_gm <- subset(PL_gm, Water2 %in% "Reference")
#plot
lakelab <- c("Barren Lake", "Semi-barren Lake", "Forested Lake")

PL1<-ggplot(subset(PL_gm, Water2 %in% c("Baby Lk.", "Daisy Lk.", "Nelson Lk.")), aes(x=Water2, y=gemmean, colour=TAXON))+
  geom_errorbar(aes(ymin=glower.CL, ymax=gupper.CL), width=0.2, show.legend=F)+
  geom_point()+
  ylim(0,1.1) +
  ylab("Proportion Littoral") + 
  scale_colour_manual(labels = c("Smallmouth bass", "Yellow perch"), values = c("#999999", "#E69F00")) +
  theme_bw()

#w reference CI
PL<- PL1+
  geom_hline(data = refPL_gm, aes(yintercept = gemmean,linetype = "Reference Lakes mean", colour=TAXON), lwd=1.1)+
     theme(axis.title.x = element_blank()) +
  geom_rect(data = refPL_gm, aes(ymin = glower.CL, ymax = gupper.CL), fill=refPL_gm$colour, xmin = -Inf, xmax = Inf, alpha = 0.2) +
  scale_fill_manual(values = c("Smallmouth bass" = "#999999",
                               "Yellow perch" = "#E69F00"), guide = "none")+
   theme(axis.title.x = element_blank()) +
  scale_x_discrete(labels = lakelab, limits = c('Baby Lk.','Daisy Lk.','Nelson Lk.'))
...
TP_gm$colour<-"#999999"
TP_gm$colour[TP_gm$TAXON=="YPERCH"] <- "#E69F00"
TP_gm$Water2<-as.factor(TP_gm$Water2)
TP_gm$TAXON<-as.factor(TP_gm$TAXON)
refTP_gm<-subset(TP_gm, Water2 %in% "Reference")

#plot
TP1<- ggplot(subset(TP_gm, Water2 %in% c("Baby Lk.", "Daisy Lk.", "Nelson Lk.")), aes(x=Water2, y=gemmean, colour=TAXON))+
  geom_errorbar(aes(ymin=glower.CL, ymax=gupper.CL), width=0.2,show.legend=F)+
  geom_point()+
  ylim(2.9,4.1)+
  ylab("Trophic Position")+
  scale_colour_manual(name = "Species", labels = c("Smallmouth bass", "Yellow perch"), values = c("#999999", "#E69F00")) +
  theme_bw()
#w reference CI
TP<- TP1+
  geom_hline(data = refTP_gm, aes(yintercept = gemmean,linetype = "Species grand mean", colour=TAXON), lwd = 1.1) +
      scale_linetype_manual(name = "Reference Lakes", values = c(1,1))+
  geom_rect(data = refTP_gm, aes(ymin = glower.CL, ymax = gupper.CL), fill=refTP_gm$colour, xmin = -Inf, xmax = Inf, alpha = 0.2, show.legend=F)+
   theme(axis.title.x = element_blank()) +
  scale_x_discrete(labels = lakelab, limits = c('Baby Lk.','Daisy Lk.','Nelson Lk.'))
...
rm(smb_pl_mod, yp_pl_mod, smb_tp_mod, yp_tp_mod, smb_pl_em, yp_pl_em, smb_tp, yp_tp, mod_means)
```
...
####NEW #######
#rename columns to specify it's all gmean for PL and TP to set up for SEA ellipses
TP_gm <- TP_gm %>%
  #rename columns - do same for PL
```
