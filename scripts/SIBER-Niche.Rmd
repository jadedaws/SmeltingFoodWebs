...
#siberdat <- read_csv("../data/siberdat.csv")
#siberdat <- siberdat[-c(1)]
siberdat <- data.frame(
  iso1 = bigdat$adjprop_litt_EZ,
  iso2 = bigdat$tp_2,
  group = bigdat$TAXON,
  community = bigdat$Water
  )

siberdat <- siberdat %>%
  filter(group %in% c("SMB", "YPERCH"))
...
# defining ellipses. dy = Daisy Lake, bb = Baby Lake, nl = Nelson Lake, td = Tadenac Lake, rc = Richardson Lake; yp = yellow perch, smb = smallmouth bass.
#dy smb
ellipse1 <- "1.1" 
#dy yp
ellipse2 <- "1.2"
...
ggplot(siberdat, aes(x = iso1, y = iso2, colour = group)) +
  geom_point() +
